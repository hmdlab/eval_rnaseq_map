...
.calc_row <- function(annotation, label_) {
  ...
  .pct_uexons_gene <- .uexons_gene$value / .exons_gene$value * 100
  ...
  .row <-
    list(
      ...
      avg_exons_transcript = mean(.exons_transcript$value),
      avg_exons_gene = mean(.exons_gene$value),
      avg_uexons_gene = mean(.uexons_gene$value),
      avg_pct_uexons_gene = mean(.pct_uexons_gene)
    ) %>%
    enframe %>%
    unnest(value) %>%
    rename(feature = name) %>%
