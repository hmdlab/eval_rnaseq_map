...
def calc(opt):
    ...
    paths_fastq = sorted(
        [
            path
            for path in glob(f"{path_work_dir}/*")
            for p in [".fastq", ".mate1", ".mate2"]
            if p in os.path.splitext(path)[1]
        ]
    )
    ...
    rendered = template.render(tsv_aligned=path_tsv, tsv_unaligned=path_tsv_unaligned)
...
def agg(opt):
    ...
    path_output = os.path.join(
        os.path.dirname(paths_sqlite[0]), "eval_align.merged.sqlite"
    )
