...
plots_main_quant_annotated <- .data$plot
...
## 7. Arrange plots (align, quant, and de)
...
g <- ggpubr::ggarrange(
  plots_main_de[[3]] %>% .add_lab("(A)"),
  ggpubr::ggarrange(plots_top_align$f1 %>% .add_lab("(B)"),
                    NULL,
                    ncol = 2, widths = c(1.3, 0.7)),
  plots_main_quant_annotated[[1]] %>% .add_lab("(C)"),
  ncol = 1,
  nrow = 3,
  align = "hv",
  heights = c(1.2, 0.6, 1.2),
  common.legend = FALSE,
  legend = "bottom"
)

g %>% utl$save_plot("f1_spearman", "arranged_de_align_quant.pdf", width = 3.2 * 3, height = 3.2 * 3.4)
...
## 9. Additonal experiment: The impact of tailoring annotation
...
inputs_tailored <- bind_rows(
  data.frame(
    dataset = "sim_add",
    condition = c(0),
    input_dir = here::here("results", c("testXX_main")),
    stringsAsFactors = FALSE
  )
)
```
...
## 10. Additonal experiment: FPs vs. mappability

### 10.1 Preparetion
```{r}
tx_gene <- annotation_main %>%
  distinct(gene_id, transcript_id) %>%
  filter(!is.na(transcript_id))

result_quant_gene_true_cpm <- result_quant_true_cpm %>%
  left_join(
    tx_gene,
    by = c(feature_id = "transcript_id")
  ) %>%
  group_by(gene_id) %>%
  summarize(ctrl = sum(ctrl), case = sum(case)) %>%
  # mutate(mean = (ctrl + case) / 2) %>%
  pivot_longer(-1, names_to = "sample", values_to = "cpm")


threshold <- 0.25

.feature_ids_fps_quant <- results_quant_est_cpm$data %>%
  map(
    .,
    ~ pivot_longer(.x,-1, names_to = "sample",  values_to = "cpm") %>%
      filter(cpm > threshold) %>%
      unite(col = id, c(feature_id, sample), sep = "@") %>%
      pull(id) %>%
      setdiff(.,
               result_quant_true_cpm %>%
                 pivot_longer(-1, names_to = "sample", values_to = "cpm") %>%
                 unite(col = id, c(feature_id, sample), sep = "@") %>%
                 pull(id)
      )
  ) %>%
  set_names(results_quant_est_cpm$combination)

feature_ids_fps_quant <- .feature_ids_fps_quant %>% map(
  ~ data.frame(id = .) %>%
    separate(
      col = id,
      into = c("feature_id", "sample"),
      sep = "@"
    ) %>%
    left_join(tx_gene,
              by = c(feature_id = "transcript_id")) %>%
    left_join(result_quant_gene_true_cpm, by = c("gene_id", "sample")) %>%
    left_join(
      mappabilities %>%
        select(feature_type, data) %>%
        deframe %>%
        .$transcript %>%
        select(feature_id, mappability),
      by = "feature_id"
    ) %>%
    rename(mappability_tx = mappability) %>%
    left_join(
      mappabilities %>%
        select(feature_type, data) %>%
        deframe %>%
    ...
    rename(mappability_gene = mappability) %>%
  rename(cpm_gene = cpm)
)
...
### 10.2 Draw plots
```{r}
.data <- feature_ids_fps_quant

.draw <- function(name, data, xlab, x) {
  .theme <- theme(
    text = element_text(size = 18),
    legend.position = "bottom",
    legend.title = element_blank(),
...
.data <- .data %>%
  ...
  mutate(plot_cpm_gene = map2(
    name,
    data,
    ~ .y %>%
      mutate(cpm_gene = replace_na(cpm_gene, 0)) %>%
      .draw(.x, ., "CPM (gene-level)", "cpm_gene")
  ))

.arrange <- function(list_plots) {
  .plotlist <- map2(list_plots,
                    paste0("(", LETTERS, ")")[1:length(list_plots)],
                    ~ utl$add_lab(.x, .y))
...
map2(
  .data %>% select(starts_with("plot")) %>%
    apply(2, .arrange),
  .data %>% select(starts_with("plot")) %>% colnames,
  ~ .x %>% utl$save_plot(
    .y,
    "feature_ids_fps_hist_quant.pdf",
    width = 3.2 * 3.2,
    height = 3.2 * 3
  )
)
