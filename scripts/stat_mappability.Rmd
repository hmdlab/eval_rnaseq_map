...
.extract_features <- function(x) {
  ...
  feature_ids_dup %>% length %>% message

  y <- y %>% filter(!(feature_id %in% feature_ids_dup & feature_type == ""))
...
features_gencode <- features_gencode %>%
  ...
  mutate(feature_type = ifelse(!(feature_type %in% c(".lncrna", ".protein_coding")), ".others", feature_type))
...
features_others <-
  ...
  map(
    ~ .x %>%
      ...
      mutate(
        feature_type = ifelse(!(feature_type %in% c(".lncrna", ".protein_coding")), ".others", feature_type)
      )
  )
...
.mean_mappability <- function(x) {
  ...
  .others <- x %>% filter(biotype == ".others") %>% .$mappability %>% mean

  list(all = .all, mrna = .pc, lncrna = .lncrna, others = .others) %>%
    enframe %>%
    unnest(value) %>%
    rename(biotype = name) %>%
    rename(mean = value)
...
feature_ids_curated <- c(annotations[["GENCODE"]] %>% filter(source %in% "HAVANA") %>% pull(transcript_id) %>% unique,
                         annotations[["RefSeq"]] %>% filter(source %in% c("BestRefSeq", "Curated Genomic")) %>% pull(transcript_id) %>% unique)

.n <- function(x) {
  .all <- x %>% nrow
  .pc <- x %>% filter(biotype == ".protein_coding") %>% nrow
  .lncrna <- x %>% filter(biotype == ".lncrna") %>% nrow
  .others <- x %>% filter(biotype == ".others") %>% nrow
...
.draw <- function(name, data, feature_type, x) {
  ...
  g <- data %>%
    filter(biotype %in% c(".protein_coding", ".lncrna", ".others")) %>%
  ...
  g <- g + .theme
  g <-
    g + scale_colour_manual(values = c(mRNA = utl$to_color("mRNA"), lncRNA = utl$to_color("lncRNA"), Others = utl$to_color("Others")))
  g <-
    g + scale_fill_manual(values = c(mRNA = utl$to_color("mRNA"), lncRNA = utl$to_color("lncRNA"), Others = utl$to_color("Others")))
}
...
### 5.3. Curation methods separately
```{r}
.data <- mappabilities %>%
  filter(feature_type == "transcript") %>%
  filter(name %in% c("GENCODE", "RefSeq")) %>%
  mutate(data = map(data,
                    ~ .x %>% mutate(
                      curated = factor(ifelse(feature_id %in% feature_ids_curated, TRUE, FALSE))
                    )))
...
.plots_all %>%
  ggsave(plot = ., file = file.path(OUTPUT_DIR, "mappabilities_curated_not_hist.pdf"), width = 9.6 * 2, height = 6.4 * 1)
...
.plot_arranged %>%
  ggsave(plot = ., file = file.path(OUTPUT_DIR, "mappabilities_curated_hist.pdf"), width = 6.4 * 2, height = 6.4 * 2)
